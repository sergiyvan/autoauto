...
void KalmanObstacleTracker::updateHook()
{
    ...
    if(counter < maxSize){
        tempR(counter,0) = mZ[0];
        tempR(counter,1) = mZ[1];
        counter++;
    }else{
        std::cout << "================"<< std::endl;
        std::cout << "================"<< std::endl;
        Eigen::Matrix2d k = tempR.transpose() *tempR;
        Eigen::MatrixXd x_mean = tempR.colwise().mean();
        mR = (1./(maxSize-1))*(k-(maxSize*(x_mean.transpose()*x_mean)));
